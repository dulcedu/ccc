...
contract MultiSigWallet {
  ...
  modifier onlyWallet() {
    require(
      ...
      "MultiSigWallet.onlyWallet : Only this wallet can call this function"
    );
    _;
  }
  ...
  {
    for (uint i = 0; i < owners.length; i++) if (owners[i] == owner) {
      owners[i] = newOwner;
      break;
    }
    isOwner[owner] = false;
    ...
    notExecuted(transactionId)
  {
    if (isConfirmed(transactionId)) {
      ...
      // Originally we had:
      // if (tx.destination.call.value(msg.value)(msg.data)) 
      bool success = transaction.destination.transfer(msg.value);
      if (success)
  ...
  function getTransactionCount(bool pending, bool executed)
    ...
    returns (uint count)
  {
    for (uint i = 0; i < transactionCount; i++) if (pending && !transactions[i].executed || executed && transactions[i].executed) count += 1;
  }
    ...
    returns (address[] memory _confirmations)
  {
    ...
    uint count = 0;
    uint i;
    for (i = 0; i < owners.length; i++) if (confirmations[transactionId][owners[i]]) {
    ...
    returns (uint[] memory _transactionIds)
  {
    ...
    uint count = 0;
    uint i;
    for (i = 0; i < transactionCount; i++) if (pending && !transactions[i].executed || executed && transactions[i].executed) {
...
contract MultiSigWalletWithDailyLimit is MultiSigWallet {
  ...
  {     
    ...
    if (confirmed || msg.data.length == 0 && isUnderLimit(msg.value)) {
      ...
      if (transaction.destination.transfer(msg.value)) 
        emit Execution(transactionId);
      else {
        emit ExecutionFailure(transactionId);
        transaction.executed = false;
        if (!confirmed) spentToday -= msg.value;
      }
    }
  }
